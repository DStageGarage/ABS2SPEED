...
// Set to 1 if speed limit is to be used (requires setting the speedometr limit MAX_SPEEDO_FREQ!)
#define USE_SPEEDO_LIMIT 0
// Set to 1 if staging is to be TURNED ON (requires setting the speedometr limit MAX_SPEEDO_FREQ!)
...
ISR(TIMER3_CAPT_vect)
{
  if(t3_ovf > 1)
  {
    //timestamp = ICR3;                                 // save new timestamp - moved to the end
    // clear overflow and wait till next event
    t3_ovf = 0;
    //return;                                           // exit interrupt - swapped to else statement
  }
  else
  {
    if(t3_ovf == 1)
    {
      // if there was one overflow calculate time from last capture to overflow and add current capture
...
ISR(TIMER3_OVF_vect)
{
  ...
  {
    t3_ovf = 2;
    period = 0;                                         // redundand to main loop, just in case, unwanted behaviour possible without it with glithces at the input
  }
}
// =========================================================================================
...
ISR(TIMER4_COMPA_vect)
{
  ...
  if(period == 0)
  {
    ...
    // leave output state as it is without transition
...
// =========================================================================================
  ...
  float dipswitch_setup_and_read()
  {
    ...
    return (float)temp * 0.005;                         // factor equals dipswitch decimal value * 0.01 but internaly factor has to be divided by 2 thus 0.005
  }
#endif
// =========================================================================================
